...
- New `src/lib/performance/compare.ts`: a metric direction map (`lower_better` for time, `higher_better` for rounds/reps/distance/intervals/load-lifted, `neutral` for RPE and training load) plus a `compareAttempts` helper that returns `not_comparable` when the two attempts carry different prescription fingerprints. All attempt deltas and the Progress trend go through it; unit tests cover each direction and the version-change case.
- New `src/lib/workout/prescription-fingerprint.ts`: stable hash over performance-relevant plan fields only (format, ordered exercises, reps/time/distance, intervals, work/rest, rounds, duration) — ignoring name, description, tips and image. Stored per attempt as `prescription_hash` on `workout_results` (added in the same migration) so historical attempts keep the version they were performed under.
